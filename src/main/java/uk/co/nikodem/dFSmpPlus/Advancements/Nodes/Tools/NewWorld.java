...
public class NewWorld extends BaseAdvancement {
    public static AdvancementKey KEY = new AdvancementKey(ADVANCEMENT_NAMESPACE, "new-world");

    public NewWorld(@NotNull Advancement parent) {
        super(KEY.getKey(),
                new AdvancementDisplay.Builder(
                        DFMaterial.PointyStick.toItemStack(),
                        "New World")
                        .description("Chisel a log with a pointy stick.")
                        .taskFrame()
                        .coords(1f, 0f)
                        .showToast()
                        .announceChat()
